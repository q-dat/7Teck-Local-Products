...
const ACTIVE_CATEGORY_STORAGE_KEY = "local-products-active-category-v2";

const getActiveCategoryStorageKey = (scope: LocalPageScope): string =>
  `${ACTIVE_CATEGORY_STORAGE_KEY}-${scope}`;

const loadActiveCategoryTab = (scope: LocalPageScope): CategoryTab => {
  if (typeof window === "undefined") return "all";

  try {
    const storedCategory = window.localStorage
      .getItem(getActiveCategoryStorageKey(scope))
      ?.trim();
...
const saveActiveCategoryTab = (
  scope: LocalPageScope,
  category: CategoryTab,
): void => {
  if (typeof window === "undefined") return;

  try {
    window.localStorage.setItem(getActiveCategoryStorageKey(scope), category);
  } catch {
    return;
  }
};
...
export default function LocalPage({
  ...
  const scopedContentType: LocalPageScope = scope;
  ...
  const viewProducts = useMemo(
    () =>
      products.filter(
        (product) => product.contentType === scopedContentType,
      ),
    [products, scopedContentType],
  );
  ...
  useEffect(() => {
    setActiveCategoryTab(loadActiveCategoryTab(scopedContentType));
  ...
  useEffect(() => {
    ...
    saveActiveCategoryTab(scopedContentType, activeCategoryTab);
  }, [activeCategoryTab, isActiveCategoryReady, scopedContentType]);
  ...
  useEffect(() => {
    ...
    setSettings((current) => {
      const currentKeys = new Set(
        current.categoryOrder.map((category) => normalizeTextKey(category)),
      );
      const missingCategories = categories.filter(
        (category) => !currentKeys.has(normalizeTextKey(category)),
      );
      ...
      return {
        ...current,
        categoryOrder: [...current.categoryOrder, ...missingCategories],
      };
    });
  }, [categories]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    const matchedProducts = viewProducts.filter((product) => {
  ...
  }, [activeCategoryTab, categories, pendingDoneProductIds, query, viewProducts]);
  ...
  const productRenderKey = useMemo(() => {
    return [
      ...
      query,
      viewProducts.length,
      viewProducts[0]?.updatedAt ?? "",
      categories.join("\u0000"),
    ].join("\u0001");
  }, [activeCategoryTab, categories, query, viewProducts]);
  ...
  useEffect(() => {
    ...
    setPostedRecords((current) => {
      ...
      const nextRecords = current.map((record) => {
        ...
        const taskIndex = Number(taskNumberText) - 1;
        const legacyProduct = viewProducts.find(
  ...
  }, [scheduleAssignments, scheduleTimes, viewProducts]);
  ...
  const assignProductToSchedule = (
    ...
    if (productId) {
      const selectedProduct = viewProducts.find(
          ...
          <div className="relative grid w-full grid-cols-1 gap-2 xl:flex xl:flex-col xl:items-stretch xl:gap-1.5 xl:p-2">
            ...
            >
              ...
              <button
                ...
                aria-pressed={activeModal === "contentNavigation"}
                className={`${headerActionButtonBaseClassName} ${activeModal === "contentNavigation"
                  ? headerActiveButtonClassName
                  : headerNeutralButtonClassName
                  }`}
                onClick={() => openModal("contentNavigation")}
              >
